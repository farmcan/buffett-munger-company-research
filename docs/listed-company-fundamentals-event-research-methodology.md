...
当一家公司存在多个重叠催化，或需要横向观察多家公司时，不能只交付一张日期表。公开 HTML
应提供可下钻的事件终端：

- 所有公司/证券使用共享日期轴，并以 `NOW` 明确分隔历史和未来；
- 每家公司一条泳道；实心节点表示已发生，空心节点表示待发生或待确认；
- 节点大小只表示研究影响力，不表示预期收益；
- 每个事件按 `冻结预期 → 官方实际 → Surprise → T0/T+1/T+5/T+20
  → 行业/资本结构传导 → 下一验证` 展开；
- 没有事前冻结共识时必须标记 `not_comparable`，不能用同比增速事后制造 Surprise；
- 相邻公告、产品发布、治理或融资必须合并为 `event_cluster` 或明确写出重叠限制；
- 未来节点只冻结问题和官方日期状态，不预填结果、价格方向或伪未来 K 线；
- 跨公司页面可使用共同风险偏好指数作背景，但必须标明它不是所有公司的共同业绩基准；
- 每个数字和状态都绑定原始 URL；主源、行情源和研究推断分层。

标准数据状态为：

| 状态 | 含义 | 图形 |
| --- | --- | --- |
| `scheduled` | 日期或窗口待发生，预期尚未冻结 | 空心节点 |
| `expectation_frozen` | 事前问题/指引已冻结，只能追加实际 | 加粗空心节点 |
| `actual_reported` | 官方实际已披露，市场窗口未完整复核 | 实心节点 |
